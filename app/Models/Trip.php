...
final class Trip {
  public static function listPublic(): array {
    $sql = "SELECT t.*, a1.name AS from_name, a2.name AS to_name,
  ...
  public static function listAll(): array {
    return DB::pdo()->query('SELECT * FROM trips ORDER BY id DESC')->fetchAll();
  }
  public static function find(int $id): ?array {
    $s = DB::pdo()->prepare('SELECT * FROM trips WHERE id=?');
  ...
  public static function delete(int $id, int $authorId, bool $force=false): void {
    if ($force) DB::pdo()->prepare('DELETE FROM trips WHERE id=?')->execute([$id]);
    else DB::pdo()->prepare('DELETE FROM trips WHERE id=? AND author_id=?')->execute([$id,$authorId]);
  }
}
